...
echo "Starting CycloneDX CLI installation..."

# Determine OS
OS="$(uname -s | tr '[:upper:]' '[:lower:]')"
ARCH="$(uname -m)"

# Map OS name to CycloneDX release format
if [ "$OS" = "darwin" ]; then
    OS="osx"
elif [ "$OS" = "linux" ]; then
    OS="linux"
...
# Map Architecture name to CycloneDX release format
...
if [ "$OS" = "win" ]; then
    ASSET_NAME="cyclonedx-win-${ARCH}.exe"
else
    ASSET_NAME="cyclonedx-${OS}-${ARCH}"
fi
...
LATEST_RELEASE_URL="https://api.github.com/repos/CycloneDX/cyclonedx-cli/releases/latest"
...
if [ -z "$DOWNLOAD_URL" ]; then
    ...
    echo "Please check https://github.com/CycloneDX/cyclonedx-cli/releases/latest"
    exit 1
fi
...
if [[ "$ASSET_NAME" == *.zip ]] || file "$ASSET_NAME" | grep -q 'Zip archive'; then
    echo "Archive detected. Unzipping..."
    unzip -q "$ASSET_NAME"
    # Find the binary after unzipping
    if [ "$OS" = "win" ]; then
        BIN_FILE="cyclonedx.exe"
    else
        BIN_FILE="cyclonedx"
    fi
else
    # The releases from CycloneDX are typically straight binaries
    BIN_FILE="$ASSET_NAME"
fi
...
FINAL_BIN_NAME="cyclonedx"
if [ "$OS" = "win" ]; then
    FINAL_BIN_NAME="cyclonedx.exe"
...
if command -v cyclonedx >/dev/null 2>&1; then
    ...
    echo "=========================================================="
    echo " cyclonedx binary was installed to $INSTALL_DIR,"
